...
isl.plot <- ggplot(C.fort[is.na(C.fort$REALM) == FALSE,], aes(long, lat, group=group)) +
 ...
 # geom_polygon(data=LINES,  aes(long, lat, group=group), 
  ...
  geom_polygon(aes(fill=as.factor(INSULARITY)), colour="darkgrey", size=.2) + 
  geom_point(data=PRED.PLOTS, aes(x=X, y=Y, group=NULL), shape=3, colour="black") +
...
# export the figure
png("../Figures/obs_vs_pred.png", width=2000, height=950, res=250)
obs.pred.brm
dev.off()
...
data.frame(1:56, summary(brm.SMOOTH$fit)$summary[,'50%'])

all.vars.SMOOTH <- standata(brm.SMOOTH)$X

all.splines.SMOOTH <- standata(brm.SMOOTH)$Zs_2_1
pars.country.splines.SMOOTH <- summary(brm.SMOOTH$fit)$summary[,'50%'][30:42]
pars.plot.splines.SMOOTH <- summary(brm.SMOOTH$fit)$summary[,'50%'][43:55]
pars.envivars.SMOOTH <- summary(brm.SMOOTH$fit)$summary[,'50%'][5:26]
...
for.pairs <- dplyr::select(DAT, S, Tree_dens, min_DBH, GPP, ANN_T, ISO_T, 
                           MIN_P, P_SEAS, ALT_DIF, ELONGATION, INSULARITY, REALM, DAT_TYPE)
for.pairs$ISLAND <- as.factor(for.pairs$INSULARITY)
p <- ggpairs(for.pairs, aes(colour=DAT_TYPE)) + theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

# save to a file
png("../Figures/Fig_S7_pairplot.png", width=2000, height=2300, res=100)
p
dev.off()
