...
plot.gr.S <- ggplot(grid5.mlf, aes(long, lat, group=group)) +
  ...
  xlab("") + ylab("") +
  #ggtitle("A") +  
  labs(subtitle = expression(S[hex] ~ "(richness in 209,903" ~ km^2 ~ "hexagons)")) +
...
plot.pl.S <- ggplot(MAINL, aes(long, lat, group=group)) +
  ...
  xlab("") + ylab("") +
  #ggtitle("B") +  
  labs(subtitle = expression(S[plot] ~ "(richness in 1 ha plots)")) +
  theme_minimal() + blank.theme
...
plot.beta.S <- ggplot(MAINL, aes(long, lat, group=group)) +
  ...
  xlab("") + ylab("") +
  #ggtitle("C") +  
  labs(subtitle = expression(beta ~ "=" ~ S[hex]/S[plot])) +
  theme_minimal() + blank.theme
